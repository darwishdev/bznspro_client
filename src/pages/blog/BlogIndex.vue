    ...
    <div class="main q-my-xl">
      <div class="items-center flex no-wrap q-my-xl">
        ...
        <q-tabs dense v-model="tab" active-bg-color="blue" active-color="white" outside-arrows inline-label
          ...
          <q-tab v-for="cat in Object.keys(globalStore.blogCategoriesMap)" :key="cat" :name="cat" :label="cat" />
        </q-tabs>
      </div>
      <q-tab-panels v-for="cat in Object.keys(globalStore.blogCategoriesMap)" :key="cat" v-model="tab" animated>
        <q-tab-panel v-for="post in globalStore.blogCategoriesMap[cat]" :key="post.blogId" :name="cat">
